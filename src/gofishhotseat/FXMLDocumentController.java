...
public class FXMLDocumentController implements Initializable {
    ...
    @FXML
    private void tryGameStart(ActionEvent event) {
    ...
    private void startGameWindow(){
        try{
            ...
            this.updateHand();
            this.updatePlayer();
            //hand1.setImage(new Image("file:/src/artAssets/7H.png"));
        }catch(IOException ie){
            System.out.println("Error. You fool. Starting a new game window has failed.");
        }
    }
    //quick fix. relocate later
    boolean goodStart = false;
    
    @FXML
    private void startFix(MouseEvent m){
        if(!goodStart){
            updateHand();
            updatePlayer();
            goodStart = true;
